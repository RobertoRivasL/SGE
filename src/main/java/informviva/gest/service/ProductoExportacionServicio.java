 ...
 */
@Service
public class ProductoExportacionServicio {

    private static final Logger logger = LoggerFactory.getLogger(ProductoExportacionServicio.class);
    ...
    public byte[] exportarProductosBajoStock(List<Producto> productos, int umbral) {
        try (Workbook workbook = new XSSFWorkbook();
            ...
            for (Producto producto : productos) {
                ...
                stockCell.setCellValue(producto.getStock());
                if (producto.getStock() <= umbral / 2) {
                    stockCell.setCellStyle(alertStyle); // Alerta crítica
                }
